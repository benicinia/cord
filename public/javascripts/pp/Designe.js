EZP.namespace("widget");
if (typeof EZD == "undefined" || typeof EZE == "undefined") {
    EZD = EZ.util.Dom;
    EZE = EZ.util.Event;
}
EZP.widget.hideShow = {
    ...
    init: function() {
        this.containers = EZD.getElementsByClassName("hideShow");
        for (var i = 0; i < this.containers.length; i++) {
            if (!(EZD.hasClass(this.containers[i], "opened"))) {
                EZD.addClass(this.containers[i], "hide");
            }
            var reg = new RegExp("#" + this.containers[i].id);
            checkHrefs = function(el) {
                return (el.getAttribute("href").match(reg));
            }
            this.links = EZD.getElementsBy(checkHrefs, "a");
            this.cancelButtons = EZD.getElementsByClassName("closer", "*", this.containers[i]);
            ...
            for (var j = 0; j < this.links.length; j++) {
                ...
                if (!(EZD.hasClass(this.containers[i], "opened")) && EZD.hasClass(this.links[j], "opened")) {
                    this.links[j].wasOpened = true;
                    EZD.removeClass(this.links[j], "opened");
                }
                if (this.mouseOverMode) {
                    EZE.addListener(this.links[j], 'mouseover', EZP.widget.hideShow.toggleHideShow);
                } else {
                    EZE.addListener(this.links[j], 'click', EZP.widget.hideShow.toggleHideShow);
                }
            }
            if (this.cancelButtons.length > 0) {
                for (var k = 0; k < this.cancelButtons.length; k++) {
                    ...
                    EZE.addListener(this.cancelButtons[k], 'click', EZP.widget.hideShow.toggleHideShow);
                }
            }
        }
    },
    hide: function(obj) {
        EZD.addClass(obj, "hide");
        EZD.removeClass(obj, "opened");
        if (obj && obj.hideShowLinks) {
            for (var i = 0; i < obj.hideShowLinks.length; i++) {
                if (EZD.hasClass(obj.hideShowLinks[i], "opened")) {
                    EZD.removeClass(obj.hideShowLinks[i], "opened");
    ...
    show: function(obj) {
        ...
        EZD.removeClass(obj, "accessAid");
        EZD.removeClass(obj, "hide");
        EZD.addClass(obj, "opened")
        if (obj && obj.hideShowLinks) {
            for (var i = 0; i < obj.hideShowLinks.length; i++) {
                if (obj.hideShowLinks[i].wasOpened) {
                    EZD.addClass(obj.hideShowLinks[i], "opened");
                    obj.hideShowLinks[i].wasOpened = false;
                }
            }
        }
        var buttons = EZD.getElementsBy(function(elem) {
    ...
    toggleHideShow: function(e) {
        EZE.preventDefault(e);
        var anchor = this;
        var div = anchor.hideShowContainer;
        var ignoreClick = false;
        if (EZP.widget.hideShow.mouseOverMode) {
        ...
        if (EZD.hasClass(div, "opened") && EZP.widget.hideShow.mouseOverMode && !ignoreClick) {} else if (EZD.hasClass(div, "opened")) {
            EZP.widget.hideShow.hide(div);
        } else {
            EZP.widget.hideShow.show(div);
        }
    }
}
EZE.onDOMReady(EZP.widget.hideShow.init);
var onAccordionComplete = new EZ.util.CustomEvent('onAccordionComplete');
EZP.widget.Accordion = {
    toggleReady: null,
    allowMultiple: false,
    animate: true,
    animDelayShow: 0.9,
    ...
    init: function() {
        ...
        var accordions = EZD.getElementsByClassName('accordion');
        if (!accordions) {
            return;
        }
        EZD.addClass(accordions, 'dynamic');
        for (i = 0; i < accordions.length; i++) {
            boxes = EZD.getElementsByClassName('box', '', accordions[i]);
            for (i = 0; i < boxes.length; i++) {
                header = EZD.getElementsByClassName('header', 'div', boxes[i])[0];
                body = EZD.getElementsByClassName('body', 'div', boxes[i])[0];
                body.defaultHeight = body.offsetHeight;
                EZD.setStyle(body, 'width', body.offsetWidth);
                header.content = body;
                if (EZD.hasClass(boxes[i], 'defaultOpen')) {
                    autoShow = header.content;
                }
                EZE.addListener(header, 'mousedown', EZP.widget.Accordion.toggle);
                EZE.addListener(header, 'mouseover', function(e) {
                    EZD.addClass(this, 'hover');
                });
                EZE.addListener(header, 'mouseout', function(e) {
                    EZD.removeClass(this, 'hover');
                });
            }
            if (autoShow) {
                EZP.widget.Accordion.show(autoShow);
            }
        }
    },
    toggle: function(body) {
        if (this.content)
            body = this.content;
        var accordion = EZP.widget.Accordion;
        accordion.toggleReady = (accordion.toggleReady === null) ? true : accordion.toggleReady;
        if (EZD.hasClass(body.parentNode, 'open') && accordion.toggleReady) {
            body.defaultHeight = body.offsetHeight;
    ...
    toggleCustom: function() {
        ...
        if (!box.open) {
            EZD.removeClass(box, 'open');
        } else {
            EZD.setStyle(body, 'height', 'auto');
        }
        EZP.widget.Accordion.toggleReady = true;
        if (!EZP.widget.Accordion.firstRun) {
            onAccordionComplete.fire(true);
        } else {
            EZP.widget.Accordion.firstRun = false;
        }
    ...
    show: function(body) {
        if (!EZP.widget.Accordion.allowMultiple) {
            EZP.widget.Accordion.hideAll();
        }
        EZD.addClass(body.parentNode, 'open');
        if (EZP.widget.Accordion.animate) {
            var attributes = {
                height: {
                    from: 0,
            ...
            var anim = new EZ.util.Anim(body,attributes,EZP.widget.Accordion.animDelayShow,EZ.util.Easing.backOut);
            anim.animate();
            anim.onComplete.subscribe(EZP.widget.Accordion.toggleCustom);
        } else {
            var box = body.parentNode;
            box.open = box.open ? false : true;
            if (!box.open) {
    ...
    hide: function(body) {
        if (EZP.widget.Accordion.animate) {
            ...
            var anim = new EZ.util.Anim(body,attributes,EZP.widget.Accordion.animDelayHide,EZ.util.Easing.easeBoth);
            anim.animate();
            anim.onComplete.subscribe(EZP.widget.Accordion.toggleCustom);
        } else {
            var box = body.parentNode;
    ...
    hideAll: function() {
        var boxes = EZD.getElementsByClassName('box');
        for (i = 0; i < boxes.length; i++) {
            if (EZD.hasClass(boxes[i], 'open')) {
                var body = boxes[i].getElementsByTagName('div')[1];
                c.widget.Accordion.hide(body);
            }
        }
    },
    toggleAccordion: function(nodeClass) {
        var box = EZD.getElementsByClassName(nodeClass);
        if (box[0]) {
            var body = box[0].getElementsByTagName('div')[0];
            EZP.widget.Accordion.toggle(body);
        }
    },
    reedIsOpen: function(nodeClass) {
        var node = EZD.getElementsByClassName(nodeClass);
        if (node[0]) {
            return (EZD.hasClass(node[0], 'open')) ? true : false;
        } else {
            return false;
        }
    }
};
EZP.namespace("Merchant.EZEBTN");
onPricePerOption = new EZ.util.CustomEvent('onPricePerOption');
onDropdownPriceChange = new EZ.util.CustomEvent('onDropdownPriceChange');
onItemDetailsChange = new EZ.util.CustomEvent('onItemDetailsChange');
EZD = EZ.util.Dom;
EZE = EZ.util.Event;
PPW = EZP.widget;
wHideShow = EZP.widget.hideShow;
EZP.Merchant.EZEBTN.Master = {
    timeOut: 13,
    ...
    init: function() {
        ...
        if (header) {
            ...
            EZD.removeClass(header, 'accessAid');
        }
        var content = document.getElementById("jsContent");
        EZD.removeClass(content, 'accessAid');
        var links = EZD.getElementsByClassName('submitPage', '', content);
        var linksLen = links.length;
        for (var i = 0; i < linksLen; i++) {
            EZE.on(links[i], 'click', BD_MASTER.submitLink);
        }
        BD_STEP1.init();
        BD_CBUTTON.init();
        setTimeout("BD_MASTER.showTimeout()", (BD_MASTER.timeOut * 60 * 1000));
        EZE.on("timeoutSubmit", 'click', BD_MASTER.submitTimeout);
        EZE.on("timeoutCancel", 'click', BD_MASTER.submitTimeout);
        if (document.getElementById('pageLoadMsg')) {
            EZD.addClass('pageLoadMsg', 'accessAid');
        }
    },
    showTimeout: function() {
        BD_MASTER.sessionLightbox = new EZP.util.Lightbox("timeoutLightbox");
        BD_MASTER.sessionLightbox.show();
    },
    submitTimeout: function(e) {
        EZE.preventDefault(e);
        var form = document.getElementById("EZEBTNForm");
        if (!form) {
    ...
    fadeOut: function(obj) {
        function fade(ele) {
            if (!EZD.hasClass(ele, "fadedOut")) {
                EZD.addClass(ele, "fadedOut");
            }
    ...
    toggleFade: function(ele) {
        try {
            if ((ele.faded && ele.faded == true) || EZD.hasClass(ele, "fadedOut")) {
    ...
    submitLink: function(e) {
        ...
        if (EZD.hasClass(this, 'loginSubmit')) {
            onBoardingCmd.value = 'login';
        } else if (EZD.hasClass(this, 'signupSubmit')) {
            onBoardingCmd.value = 'signup';
        } else if (EZD.hasClass(this, 'upgradeSubmit')) {
            onBoardingCmd.value = 'upgrade';
        } else if (EZD.hasClass(this, 'loginSavedSubmit')) {
...
EZP.Merchant.EZEBTN.StepOne = {
    ...
    init: function() {
        ...
        if (stepOne) {
            ...
            var fades = EZD.getElementsByClassName("fadedOut", '', stepOne);
            ...
            for (var i = 0; i < inputsLen; i++) {
                ...
                if (inputType == "radio" || inputType == "checkbox") {
                    EZE.on(inputs[i], 'click', this.itemDetailsChangeHandler);
                } else {
                    if (inputType.toLowerCase() == "text") {
                        EZE.on(inputs[i], 'keydown', this.captureEnter);
                    }
                    EZE.on(inputs[i], 'change', this.itemDetailsChangeHandler);
                }
            }
            var selects = stepOne.getElementsByTagName('select');
            var selectsLen = selects.length;
            for (var i = 0; i < selectsLen; i++) {
                EZE.on(selects[i], 'change', this.itemDetailsChangeHandler);
            }
            var shippingType = document.getElementById('itemFlatShippingType');
            var shippingWeight = EZD.getElementsByClassName('itemWeight', 'div', stepOne)[0];
            if (shippingType && shippingType.checked) {
                BD_MASTER.fadeOut(shippingWeight);
            }
            this.frequencyDOM = EZD.getElementsByClassName('ddpOptionFrequency', 'select', 'optionsPriceContainer')[0];
            if (this.frequencyDOM) {
                this.frequencyDOM = this.frequencyDOM.cloneNode(true);
            }
            EZE.addListener('subscriptionBillingAmountCurrency', 'change', function() {
    ...
    donationDropDownChange: function(e) {
        ...
        if (selectedTypeValue.options[selectedTypeValue.selectedIndex].value == 'donations') {
            var content = document.getElementById("jsContent");
            EZD.replaceClass(content, 'show', 'hide');
            var header = document.getElementById("headline");
            header = header.getElementsByTagName("h2");
            EZD.replaceClass(header, 'show', 'hide');
            var pageLoadMsg = document.getElementById("pageLoadMsg");
            EZD.replaceClass(pageLoadMsg, 'accessAid', 'show');
    ...
    itemDetailsChangeHandler: function(e) {
        ...
        if (!e) {
            ...
            var subButtonTypes = EZD.getElementsByClassName('subButtonType', '', stepOne);
    ...
    itemDetailsChangeSubscriber: function(type, obj) {
        ...
        if (obj[0].button_type) {
            var stepOne = document.getElementById('stepOne');
            var productItemPrice = EZD.getElementsByClassName('products pricing', '', stepOne)[0];
            var donationItemPrice = EZD.getElementsByClassName('contributionAmount', '', stepOne)[0].parentNode;
        ...
        if (obj[0].item_price_currency || obj[0].ddp_option_currency) {
            ...
            var currencyLabels = EZD.getElementsByClassName('currencyLabel', '', "stepOne");
            var currencyLabelsLen = currencyLabels.length;
            for (var i = 0; i < currencyLabelsLen; i++) {
                currencyLabels[i].innerHTML = currencyCode;
            }
            var currencySelects = EZD.getElementsByClassName('currencySelect', '', "stepOne");
    ...
    accordionSubscriber: function(args) {
        var y = EZD.getY('stepOne') - 18;
        window.scrollTo(0, y);
    },
    priceOptionSubscriber: function(type, obj) {
        var stepOne = document.getElementById("stepOne");
        var itemPriceContainer = EZD.getElementsByClassName('products pricing', '', stepOne)[0];
        var buttonType = document.getElementById('buttonType');
        ...
        var ddpOptionCurrency = EZD.getElementsByClassName('ddpOptionCurrency', '', optionsPriceContainer);
        var ddpOptionFrequency = EZD.getElementsByClassName('ddpOptionFrequency', '', optionsPriceContainer);
        if (this.activeButtonType == 'subscriptions' && document.getElementById("dropdownPrice").checked) {
            EZD.replaceClass(ddpOptionCurrency, 'show', 'hide');
            for (i = 0,
            ln = ddpOptionFrequency.length; i < ln; i++) {
                ddpOptionFrequency[i].disabled = false;
                EZD.removeClass(ddpOptionFrequency[i], 'hide')
            }
        } else {
            EZD.replaceClass(ddpOptionCurrency, 'hide', 'show');
            for (i = 0,
            ln = ddpOptionFrequency.length; i < ln; i++) {
                ddpOptionFrequency[i].disabled = true;
                EZD.addClass(ddpOptionFrequency[i], 'hide')
            }
        }
    },
    updateProductOptions: function() {
        ...
        var itemWeight = EZD.getElementsByClassName('itemWeight')[0];
    ...
    buildGiftCertificatePreview: function() {
        ...
        if (EZD.getElementsByClassName('gcBackgroundType')[0].checked == true) {
            url += '&style_color=' + gcColor;
        } else if (EZD.getElementsByClassName('gcBackgroundType')[1].checked == true) {
    ...
    hide: function(elClass) {
        var stepOne = document.getElementById("stepOne");
        var elements = EZD.getElementsByClassName(elClass, '', stepOne);
        var elementsLen = elements.length;
        for (var i = 0; i < elementsLen; i++) {
            if (!EZD.hasClass(elements[i], 'accessAid')) {
                EZD.addClass(elements, 'accessAid');
            }
            if (EZD.hasClass(elements[i], 'accessAid')) {
    ...
    show: function(elClass) {
        ...
        for (var i = 0; i < elementsLen; i++) {
            if (EZD.hasClass(elements[i], 'accessAid')) {
                EZD.removeClass(elements[i], 'accessAid');
...
EZE.addListener('buttonType', 'change', EZP.Merchant.EZEBTN.StepOne.donationDropDownChange);
EZP.Merchant.EZEBTN.CustomizeButton = {
    numOptionsPrice: 0,
    numOptions1: 0,
    numOptions2: 0,
    numOptions3: 0,
    ...
    init: function() {
        var dropdownPrice = document.getElementById("dropdownPrice");
        if (dropdownPrice) {
            EZE.addListener(dropdownPrice, 'click', this.showDropdownPrice, this);
        }
        var dropdown = document.getElementById("dropdown");
        if (dropdown) {
            EZE.addListener(dropdown, 'click', this.showDropdown, this);
        }
        var textfield = document.getElementById("textfield");
        if (textfield) {
            EZE.addListener(textfield, 'click', this.showTextfield, this);
        }
        var addOptionPrice = document.getElementById("addOptionPrice");
        if (addOptionPrice) {
            EZE.addListener(addOptionPrice, 'click', this.addNewOptionPrice, this);
        }
        var removeOptionPrice = document.getElementById("removeOptionPrice");
        if (removeOptionPrice) {
            EZE.addListener(removeOptionPrice, 'click', this.removeOptionsPrice, this);
        }
        var saveOptionPrice = document.getElementById("saveOptionPrice");
        if (saveOptionPrice) {
            EZE.addListener(saveOptionPrice, 'click', this.saveDropdownPrice, this);
        }
        var cancelOptionPrice = document.getElementById("cancelOptionPrice");
        if (cancelOptionPrice) {
            EZE.addListener(cancelOptionPrice, 'click', this.cancelPricePerOption, this);
        }
        var editDropdownPrice = document.getElementById("editDropdownPrice");
        if (editDropdownPrice) {
            EZE.addListener(editDropdownPrice, 'click', this.editDropdownPrice, this);
        }
        var deleteDropdownPrice = document.getElementById("deleteDropdownPrice");
        if (deleteDropdownPrice) {
            EZE.addListener(deleteDropdownPrice, 'click', this.deleteDropdownPrice, this);
        }
        var addOption = EZD.getElementsByClassName("addOption");
        if (addOption) {
            EZE.addListener(addOption, 'click', this.addNewOption, this);
        }
        var saveOption = EZD.getElementsByClassName("saveOption");
        if (saveOption) {
            EZE.addListener(saveOption, 'click', this.saveDropdown, this);
        }
        var cancelOption = EZD.getElementsByClassName("cancelOption");
        if (cancelOption) {
            EZE.addListener(cancelOption, 'click', this.cancelOption, this);
        }
        var editDropdown = EZD.getElementsByClassName("editDropdown");
        if (editDropdown) {
            EZE.addListener(editDropdown, 'click', this.editDropdown, this);
        }
        var deleteDropdown = EZD.getElementsByClassName("deleteDropdown");
        if (deleteDropdown) {
            EZE.addListener(deleteDropdown, 'click', this.deleteDropdown, this);
        }
        var addNewDropdown = document.getElementById("addNewDropdown");
        if (addNewDropdown) {
            EZE.addListener(addNewDropdown, 'click', this.addNewDropdown, this);
        }
        var saveTextfield = EZD.getElementsByClassName("saveTextfield");
        if (saveTextfield) {
            EZE.addListener(saveTextfield, 'click', this.saveTextfield, this);
        }
        var editTextfield = EZD.getElementsByClassName("editTextfield");
        if (editTextfield) {
            EZE.addListener(editTextfield, 'click', this.editTextfield, this);
        }
        var deleteTextfield = EZD.getElementsByClassName("deleteTextfield");
        if (deleteTextfield) {
            EZE.addListener(deleteTextfield, 'click', this.deleteTextfield, this);
        }
        var cancelTextfield = EZD.getElementsByClassName("cancelTextfield");
        if (cancelTextfield) {
            EZE.addListener(cancelTextfield, 'click', this.cancelTextfield, this);
        }
        var addNewTextfield = document.getElementById("addNewTextfield");
        if (addNewTextfield) {
            EZE.addListener(addNewTextfield, 'click', this.addNewTextfield, this);
        }
        var buttonAppLink = document.getElementById("buttonAppLink");
        if (buttonAppLink) {
            EZE.addListener(buttonAppLink, 'click', this.toggleButtonAppSection, this);
        }
        var EZPButton = document.getElementById("EZPButton");
        if (EZPButton) {
            EZE.addListener(EZPButton, 'click', this.showEZPButtonSection, this);
        }
        var customButton = document.getElementById("customButton");
        if (customButton) {
            EZE.addListener(customButton, 'click', this.showCustomButtonSection, this);
        }
        var smallButton = document.getElementById("smallButton");
        if (smallButton) {
            EZE.addListener(smallButton, 'click', this.displaySmallImage, this);
        }
        var ccLogos = document.getElementById("ccLogos");
        if (ccLogos) {
            EZE.addListener(ccLogos, 'click', this.displayCCImage, this);
        }
        var buttonTextBuyNow = document.getElementById("buttonTextBuyNow");
        if (buttonTextBuyNow) {
            EZE.addListener(buttonTextBuyNow, 'change', this.updateImageText);
        }
        var buttonTextSubscribe = document.getElementById("buttonTextSubscribe");
        if (buttonTextSubscribe) {
            EZE.addListener(buttonTextSubscribe, 'change', this.updateImageText);
        }
        var selectCountryLanguage = document.getElementById("qunn");
        if (selectCountryLanguage) {
            EZE.addListener(selectCountryLanguage, 'change', this.updateButtonCountry, this);
        }
        var selectqun = document.getElementById("qun");
        if (selectqun) {
            EZE.addListener(selectqun, 'change', this.updateButtonCountry, this);
        }
        var country_code = document.getElementById("country_code");
        var readOnlyLabel = EZD.getElementsByClassName("readOnlyLabel");
        if (readOnlyLabel) {
            EZE.addListener(readOnlyLabel, 'focus', this.makeFieldReadOnly, this);
        }
        var createButton = document.getElementById("createButton");
        if (createButton) {
            EZE.addListener(createButton, 'click', this.saveDropdownsAndSubmit, this);
        }
        var accordionTabs = EZD.getElementsByClassName("header");
        if (accordionTabs) {
            EZE.addListener(accordionTabs, 'click', this.saveDropdowns, this);
        }
        var exampleLink = EZD.getElementsByClassName("exampleLink");
        if (exampleLink) {
            EZE.addListener(exampleLink, 'click', this.createPopupImage, this);
        }
        var defCurrency = EZD.getElementsByClassName("ddpOptionCurrency", "select", "dropdownPriceSection");
        if (defCurrency) {
            EZE.addListener(defCurrency, 'change', this.updateCurrency, this);
        }
        this.createDropdownPriceStructure();
        this.createDropdownStructure();
        this.createTextfieldStructure();
        var test = EZD.getElementsByClassName("savedDropdownSection");
        var testLen = test.length;
        var cnt = 0;
        for (var i = 0; i < testLen; i++) {
            if (EZD.hasClass(test[i], "opened"))
                cnt++;
        }
    ...
    showDropdownPrice: function() {
        ...
        if (this.checked) {
            if (dropdownPriceSection) {
                ...
                var options = EZD.getElementsByClassName("optionRow", "p", "dropdownPriceSection");
                if (options.length == 0) {
                    for (var i = 0; i < 3; i++) {
                        BD_CBUTTON.createNewPriceOption();
                    }
                }
                var optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
    ...
    showTextfield: function() {
        if (this.checked) {
            ...
            for (var tfNum = 1; tfNum < 3; tfNum++) {
                ...
                var textfieldTitle = EZ.util.Dom.get("textfieldTitle" + tfNum);
                var savedTextfield = EZ.util.Dom.get("savedTextfield" + tfNum);
                var previewTextfieldTitle = EZ.util.Dom.get("previewTextfieldTitle" + tfNum);
                if (textfieldSection) {
                ...
                if (previewTextfieldSection) {
                    ...
                    previewTextfieldTitle.innerHTML = EZ.util.Dom.get("titleStr").value;
                }
    ...
    showEZPButtonSection: function() {
        var EZPButtonSection = document.getElementById("EZPButtonSection");
        var customButtonSection = document.getElementById("customButtonSection");
        var previewImageSection = EZD.getElementsByClassName("previewImageSection")[0];
        var previewCustomImageSection = EZD.getElementsByClassName("previewCustomImageSection")[0];
        if (this.checked) {
            wHideShow.hide(customButtonSection);
            wHideShow.show(EZPButtonSection);
    ...
    showCustomButtonSection: function() {
        ...
        var previewCustomImageSection = EZD.getElementsByClassName("previewCustomImageSection")[0];
        if (this.checked) {
            wHideShow.hide(EZPButtonSection);
    ...
    toggleButtonAppSection: function(e) {
        ...
        var customizeButtonSection = EZD.getElementsByClassName("customizeButtonSection")[0];
        var borderBox = EZD.getElementsByClassName("borderBox")[0];
        if (EZD.hasClass(this, "collapsed")) {
            EZD.removeClass(this, "collapsed");
            EZD.addClass(this, "expanded");
            if (!(document.getElementById("EZPButton").checked || document.getElementById("customButton").checked)) {
                document.getElementById("EZPButton").checked = true;
            }
            if (BD_CBUTTON.buttonType == "gift_certs" || BD_CBUTTON.buttonType == "donations") {
                EZD.removeClass(borderBox, "heightFix");
            }
            wHideShow.show(buttonAppSection);
        } else if (EZD.hasClass(this, "expanded")) {
            EZD.removeClass(this, "expanded");
            EZD.addClass(this, "collapsed");
            if (BD_CBUTTON.buttonType == "gift_certs" || BD_CBUTTON.buttonType == "donations") {
                EZD.addClass(borderBox, "heightFix");
            }
    ...
    saveOptionPrice: function() {
        ...
        var optionsPriceContainer = document.getElementById("optionsPriceContainer");
        var options = EZD.getElementsByClassName("optionRow", "p", "dropdownPriceSection");
        var optionNames = EZD.getElementsByClassName("ddpOptionName", "input", "dropdownPriceSection");
        var optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
        var testDropdowns = [];
        if (buttonType != 'subscriptions') {
            var defCurrency = EZD.getElementsByClassName("ddpOptionCurrency", "select", "dropdownPriceSection")[0];
            if (defCurrency.options) {
                var defCurrencyValue = defCurrency.options[defCurrency.selectedIndex].value;
            }
        }
        optionNames = EZD.getElementsByClassName("ddpOptionName", "input", "dropdownPriceSection");
        optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
        var optionsLen = options.length;
        var selectedDropDown = document.getElementById("selectedDropDown");      
        selectedDropDown.setAttribute('value', optionsLen);
        ...
        if (buttonType != 'subscriptions') {
            options = EZD.getElementsByClassName("optionRow", "p", "dropdownPriceSection");
            if (options.length > 0) {
                var oldOptionCurrency = EZD.getElementsByClassName("ddpOptionCurrency", "", "dropdownPriceSection")[0];
                if (oldOptionCurrency.nodeName != "SELECT") {
                    ...
                    EZE.addListener(optionCurrency, 'change', BD_CBUTTON.updateCurrency, this);
                    var itemCurrencyOpts = EZD.getElementsByClassName("currencySelect", "select", "")[0];
        ...
        if (BD_CBUTTON.numOptionsPrice != 0) {
            ...
            optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
            if (buttonType == 'subscriptions') {
                optionFrequency = EZD.getElementsByClassName("ddpOptionFrequency", "select", "dropdownPriceSection");
            ...
            for (var i = 0; i < optionsLen; i++) {
                ...
                var optPriceHTMLEsc = EZP.util.escapeHTML(optPrice);
                if (buttonType == 'subscriptions') {
    ...
    cancelPricePerOption: function(e) {
        ...
        if (activeButtonType == 'subscriptions') {
            EZD.removeClass(optionFrequency, 'hide');
            optionFrequency.disabled = false;
        } else {
            EZD.addClass(optionFrequency, 'hide');
        ...
        for (var j = 0; j < ddPriceStructureLen; j++) {
            ...
            parent.setAttribute("class", "optionRow");
            parent.setAttribute("className", "optionRow");
            ...
            optionName.setAttribute("name", nm + j);
            optionName.setAttribute("class", "ddpOptionName text");
            optionName.setAttribute("className", "ddpOptionName text");
            ...
            optionPrice.setAttribute("name", nm_+j);
            optionPrice.setAttribute("class", "ddpOptionPrice text");
            optionPrice.setAttribute("className", "ddpOptionPrice text");
    ...
    removeOptionsPrice: function(e) {
        ...
        var optionList = EZD.getElementsByClassName("optionRow", "p", menuList.id);
    ...
    createNewPriceOption: function(opt) {
        ...
        if (BD_CBUTTON.numOptionsPrice < 10) {
            ...
            var optionName = document.createElement("input");
            optionName.setAttribute("type", "text");
            var nm = "ddp_option_name_"+(BD_CBUTTON.numOptionsPrice + 1);
            ...
            var optionPrice = document.createElement("input");
            optionPrice.setAttribute("type", "text");
            var nm = "ddp_option_price_"+(BD_CBUTTON.numOptionsPrice + 1);
            ...
            if (BD_CBUTTON.numOptionsPrice == 0) {
                ...
                optionCurrency.setAttribute("className", "ddpOptionCurrency");
                EZE.addListener(optionCurrency, 'change', BD_CBUTTON.updateCurrency, this);
                var itemCurrencyOpts = EZD.getElementsByClassName("qunti", "select", "")[0];
                var itemCurrencyOptsLen = itemCurrencyOpts.options.length;
                for (var i = 0; i < itemCurrencyOptsLen; i++) {
                    optionCurrency.options[i] = new Option(itemCurrencyOpts.options[i].text,itemCurrencyOpts.options[i].value);
                ...
                if (buttonTypeValue == 'subscriptions') {
                    EZD.addClass(optionCurrency, 'hide');
                }
                parent.appendChild(optionCurrency);
            } else {
                var optionCurrency = document.createElement("select");
                ...
                var defCurrency = EZD.getElementsByClassName("qunti", "select", "dropdownPriceSection")[0];
                if (defCurrency.options) {
                    var defCurrencyValue = defCurrency.options[defCurrency.selectedIndex].value;
                    optionCurrency.setAttribute("innerHTML", defCurrencyValue);
                }
    ...
    editDropdownPrice: function(e) {
        ...
        if (dropdownPriceSection) {
            wHideShow.show(dropdownPriceSection);
            var labels = EZD.getElementsByClassName("optionNameLbl", "label", "dropdownPriceSection");
            wHideShow.show(labels[0]);
            labels = EZD.getElementsByClassName("optionPriceLbl", "label", "dropdownPriceSection");
            wHideShow.show(labels[0]);
            BD_CBUTTON.displayAddOptionPriceLink();
        }
        if (previewDropdownPriceSection) {
    ...
    updateOptionsPricePreview: function() {
        ...
        var optionNames = EZD.getElementsByClassName("ddpOptionName", "input", "dropdownPriceSection");
        var optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
        var optionFrequency = EZD.getElementsByClassName("ddpOptionFrequency", "select", "dropdownPriceSection");
        var optionsLen = options.length;
    ...
    createDropdownPriceStructure: function() {
        ...
        var activeButtonType = buttonType.options[buttonType.selectedIndex].value;
        var optionFrequency = EZD.getElementsByClassName("ddpOptionFrequency", "select", "dropdownPriceSection");
        var optionCurrencies = EZD.getElementsByClassName("ddpOptionCurrency", "", "dropdownPriceSection");
        if (optionsLen != 0) {
            var temp = new Array();
    ...
    saveOption: function() {
        ...
        var dropdownTitle = EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + BD_CBUTTON.activeDropdown)[0];
        var previewDropdownTitle = EZD.getElementsByClassName("previewDropdownTitle", "label", "previewDropdownSection" + BD_CBUTTON.activeDropdown)[0];
        var addNewDropdownSection = document.getElementById("addNewDropdownSection");
        var dropdown = document.getElementById("dropdown");
        var addOption = document.getElementById("addOption" + BD_CBUTTON.activeDropdown);
        var options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + BD_CBUTTON.activeDropdown);
        var optionNames = EZD.getElementsByClassName("ddOptionName", "input", "dropdownSection" + BD_CBUTTON.activeDropdown);
        ...
        if (BD_CBUTTON.activeDropdown == 0) {
            ...
            for (var i = 0; i < 3; i++) {
                var parent = document.createElement("p");
                parent.setAttribute("class", "optionRow dropdown");
                parent.setAttribute("className", "optionRow dropdown");
                var optionName = document.createElement("input");
                optionName.setAttribute("type", "text");
                var nm = "dd" + i + "_option_name";
                optionName.setAttribute("name", nm);
                optionName.setAttribute("class", "ddOptionName text");
                optionName.setAttribute("className", "ddOptionName text");
    ...
    cancelOption: function(e) {
        ...
        var options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + ddNum);
        var optionsLen = options.length;
        var dropdown = document.getElementById("dropdown");
        for (var a = 0; a < optionsLen; a++) {
            optionsContainer.removeChild(options[a]);
        }
        EZD.getElementsByClassName("dropdownTitle", "input", dropdownSection)[0].value = BD_CBUTTON.ddStructure[ddNum - 1].title;
    ...
    displayAddOptionLink: function() {
        var options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + BD_CBUTTON.activeDropdown);
        var addOption = EZD.getElementsByClassName("addOption", "", "dropdownSection" + BD_CBUTTON.activeDropdown)[0];
    ...
    deleteDropdown: function(e) {
        ...
        for (var i = 1; i < ddStructureLen; i++) {
            if (i == ddNum) {
                ...
                var options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + i);
                var optionsLen = options.length;
                for (var aa = 0; aa < optionsLen; aa++) {
                    optionsContainer.removeChild(options[aa]);
                }
                EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + i)[0].value = "";
        ...
        for (var i = 1; i <= 4; i++) {
            ...
            if (!EZD.hasClass(savedDropdownSection, "opened")) {
                var dropdownSection = document.getElementById("dropdownSection" + i);
    ...
    recreateDropdownSection: function() {
        ...
        for (var i = 1; i < ddStructureLen; i++) {
            ...
            wHideShow.hide(dropdownSection);
            EZD.getElementsByClassName("dropdownTitle", "input", dropdownSection)[0].value = BD_CBUTTON.ddStructure[i - 1].title;
    ...
    updateSavedOptions: function(ddNum) {
        ...
        options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + BD_CBUTTON.activeDropdown);
        optionNames = EZD.getElementsByClassName("ddOptionName", "input", "dropdownSection" + BD_CBUTTON.activeDropdown);
        var optionsLen = options.length;
        if (optionsLen != 0) {
            for (var i = 0; i < optionsLen; i++) {
                var optName = optionNames[i].value;
    ...
    displayAddDropdownLink: function() {
        ...
        if (addNewDropdownSection) {
            if (BD_CBUTTON.activeDropdown != 4) {
                var dds = EZD.getElementsByClassName("dropdownSection");
                var ddsLen = dds.length;
                var ddsCnt = 0;
                for (var i = 0; i < ddsLen; i++) {
                    if (EZD.hasClass(dds[i], "opened"))
                        ddsCnt++;
                }
                if (ddsCnt == 0) {
    ...
    createDropdownStructure: function() {
        ...
        var count = 1;
        for (var i = 1; i < 5; i++) {
            var options = EZD.getElementsByClassName("optionRow", "p", "dropdownSection" + i);
            var optionsLen = options.length;
            var optionNames = EZD.getElementsByClassName("ddOptionName", "input", "dropdownSection" + i);
            var dropdownSection = document.getElementById("dropdownSection" + i);
            var savedDropdownSection = document.getElementById("savedDropdownSection" + i);
            if (optionsLen != 0) {
                var temp = new Array();
                ...
                var dropdownObj = {
                    ddNum: count,
                    title: EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + i)[0].value,
                    options: temp,
                    saved: ((EZD.hasClass(savedDropdownSection, "opened") ? true : false) || (EZD.hasClass(dropdownSection, "opened") ? true : false))
                };
    ...
    recreateSavedSection: function() {
        ...
        for (var i = 0; i < ddStructureLen; i++) {
            ...
            if (BD_CBUTTON.ddStructure[i].saved) {
                ...
                if (!(EZD.hasClass(document.getElementById("dropdownSection" + (i + 1)), "opened"))) {
    ...
    recreateSavedOptions: function(ddNum, ddActive) {
        ...
        var dropdownTitle = EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + ddActive)[0];
        savedDropdown.innerHTML = "";
        if (dropdownTitle.value != "") {
    ...
    recreateOptionsPreview: function(ddActive) {
        ...
        var options = EZD.getElementsByClassName("dropdown", "p", "dropdownSection" + ddActive);
        var optionNames = EZD.getElementsByClassName("ddOptionName", "input", "dropdownSection" + ddActive);
        var dropdownTitle = EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + ddActive)[0];
        var previewDropdownTitle = EZD.getElementsByClassName("previewDropdownTitle", "label", "previewDropdownSection" + ddActive)[0];
    ...
    addAllDropdowns: function() {
        ...
        for (var i = ddStructureLen; i < 5; i++) {
            ...
            EZD.getElementsByClassName("dropdownTitle", "input", dropdownSection)[0].value = "";
            for (var j = 1; j < 4; j++) {
                var parent = document.createElement("p");
                parent.setAttribute("class", "optionRow dropdown");
                parent.setAttribute("className", "optionRow dropdown");
    ...
    getDropdownCount: function() {
        ...
        var ddsLen = dds.length;
        var ddsCnt = 0;
        var savedDds = EZD.getElementsByClassName("savedDropdownSection");
        ...
        for (var i = 0; i < savedDdsLen; i++) {
            if (EZD.hasClass(savedDds[i], "opened"))
                savedDdsCnt++;
        }
    ...
    saveTextfield: function(e) {
        ...
        var savedTextfield = EZD.getElementsByClassName("savedTextfield", "label", "savedTextfieldSection" + BD_CBUTTON.activeTextfield)[0];
    ...
    displayAddTextfieldLink: function() {
        ...
        if (addNewTextfieldSection) {
            if (BD_CBUTTON.activeTextfield != 2) {
                var tfs = EZD.getElementsByClassName("textfieldSection");
                var tfsLen = tfs.length;
                var tfsCnt = 0;
                for (var i = 0; i < tfsLen; i++) {
                    if (EZD.hasClass(tfs[i], "opened"))
                        tfsCnt++;
                }
                if (tfsCnt == 0) {
    ...
    getTextfieldCount: function() {
        ...
        var tfsLen = tfs.length;
        var tfsCnt = 0;
        var savedTfs = EZD.getElementsByClassName("savedTextfieldSection");
        ...
        for (var i = 0; i < savedTfsLen; i++) {
            if (EZD.hasClass(savedTfs[i], "opened"))
                savedTfsCnt++;
        }
    ...
    buttonBasedChanges: function(buttonType, subButtonType, langCode) {
        var customizeButtonSection = EZD.getElementsByClassName("customizeButtonSection")[0];
        var outerContainer = EZD.getElementsByClassName("outerContainer")[0];
        var borderBox = EZD.getElementsByClassName("borderBox")[0];
        var dropdownPrice = document.getElementById("dropdownPrice");
        var dropdown = document.getElementById("dropdown");
        var textfield = document.getElementById("textfield");
        ...
        if (document.getElementById("EZPButton").checked) {
            var previewImageSection = EZD.getElementsByClassName("previewImageSection")[0];
            wHideShow.show(previewImageSection);
        } else {
            var previewCustomImageSection = EZD.getElementsByClassName("previewCustomImageSection")[0];
        ...
        switch (buttonType) {
        case "gift_certs":
            if (EZD.hasClass(document.getElementById("buttonAppLink"), "expanded")) {
                EZD.removeClass(borderBox, "heightFix");
            } else {
                EZD.addClass(borderBox, "heightFix");
            }
            EZD.setAttribute(outerContainer, 'id', 'sBox');
            if (dropdownPrice.checked) {
                dropdownPrice.click();
            }
            ...
            break;
        case "subscriptions":
            EZD.removeClass(customizeButtonSection, "heightFix");
            EZD.removeClass(borderBox, "heightFix");
            EZD.setAttribute(outerContainer, 'id', 'wideBox');
        ...
        if (BD_CBUTTON.buttonType != "view_cart") {
            if (imageUrls[locale]) {
                if (this.checked) {
                    ...
                    EZD.addClass(displayCcLogos, "inactiveChkBox");
                } else {
                    switch (BD_CBUTTON.buttonType) {
                    case "gift_certs":
                    ...
                    EZD.removeClass(displayCcLogos, "inactiveChkBox");
                }
            }
        }
    },
    ...
    updateButtonCountry: function() {
        var countryCode = document.getElementsByClassName("o1");
        var langCode = document.getElementById("qunnt");
    ...
    refreshCountry: function(results) {
        ...
        var EZEBTNForm = document.getElementById("EZEBTNForm");
        EZEBTNForm.appendChild(refreshCountryCode);
    ...
    buildDropdownInfo: function(deletedDdNum) {
        ...
        if (dropdownPrice.checked) {
            ...
            var optionPrices = EZD.getElementsByClassName("ddpOptionPrice", "input", "dropdownPriceSection");
            var optionCurrencies = EZD.getElementsByClassName("ddpOptionCurrency", "", "dropdownPriceSection");
            var optionFrequency = EZD.getElementsByClassName("ddpOptionFrequency", "", "dropdownPriceSection");
            var optionsLen = options.length;
            var previewDropdownPriceSection = document.getElementById("previewDropdownPriceSection");
            if (EZD.hasClass(previewDropdownPriceSection, "opened")) {
                if (optionsLen != 0) {
                    var dropdownObj = {
        ...
        if (dropdown.checked) {
            for (var i = 1; i < 5; i++) {
                ...
                if (EZD.hasClass(previewDropdownSection, "opened")) {
                    var options = EZD.getElementsByClassName("optionRow", "p", "dropdownSection" + i);
                    var optionNames = EZD.getElementsByClassName("ddOptionName", "input", "dropdownSection" + i);
                    var optionsLen = options.length;
                    if (optionsLen != 0) {
                        var dropdownObj = {
                            ddName: EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + i)[0].getAttribute("name"),
                            title: EZD.getElementsByClassName("dropdownTitle", "input", "dropdownSection" + i)[0].value,
                            deleted: flag,
                            withPrice: false
                        };
    ...
    saveDropdowns: function() {
        var dropdownPriceSection = document.getElementById("dropdownPriceSection");
        if (EZD.hasClass(dropdownPriceSection, "opened")) {
            BD_CBUTTON.saveOptionPrice();
        }
        for (var i = 1; i < 5; i++) {
            BD_CBUTTON.activeDropdown = i;
            var dropdownSection = document.getElementById("dropdownSection" + i);
            if (EZD.hasClass(dropdownSection, "opened")) {
                BD_CBUTTON.saveOption();
            }
            var savedDropdownSection = document.getElementById("savedDropdownSection" + i);
            if (EZD.hasClass(savedDropdownSection, "accessAid")) {
                var allInputs = dropdownSection.getElementsByTagName("input");
                var allInputsLen = allInputs.length;
    ...
    updateCurrency: function() {
        var optionCurrencies = EZD.getElementsByClassName("ddpOptionCurrency", "label", "dropdownPriceSection");
        var currencyLabels = EZD.getElementsByClassName("currencyLabel", "", "stepOne");
        var defCurrency = EZD.getElementsByClassName("ddpOptionCurrency", "select", "dropdownPriceSection")[0];
    ...
    dropdownCurrencyChange: function(type, args) {
        ...
        if (args[0].item_price_currency) {
            ...
            var currencyLabels = EZD.getElementsByClassName('ddpOptionCurrency', 'label', step1);
            var currencyLabelsLen = currencyLabels.length;
            for (i = 0; i < currencyLabelsLen; i++) {
                currencyLabels[i].innerHTML = currencyCode;
            }
            var currencySelects = EZD.getElementsByClassName('qunti', 'select', '');
            var currencySelectsLen = currencySelects.length;
            for (i = 0; i < currencySelectsLen; i++) {
...
BD_MASTER = EZP.Merchant.EZEBTN.Master;
BD_STEP1 = EZP.Merchant.EZEBTN.StepOne;
BD_CBUTTON = EZP.Merchant.EZEBTN.CustomizeButton;
EZP.util.Event.onDomReady(BD_MASTER.init);
EZP.Merchant.EZEBTN.StepTwo = {
    ...
    init: function() {
        ...
        if (!stepTwo)
            return;
        var fades = EZD.getElementsByClassName("fadedOut", '', stepTwo);
        BD_MASTER.fadeOut(fades);
        EZE.addListener("enableHostedButtons", 'click', this.toggleOptions);
        EZE.addListener("enableInventory", 'click', this.toggleTables);
        EZE.addListener("enableProfitAndLoss", 'click', this.toggleTables);
        EZE.addListener("trackByOption", 'click', this.trackByOption);
        EZE.addListener("trackByItem", 'click', this.trackByItem);
        EZE.addListener("chooseAnotherDropDown", 'click', this.chooseAnotherDropDown);
        EZE.addListener("ddLightboxSubmit", 'click', this.pickDropDown);
        EZE.addListener("enablePreOrder", 'click', this.enablePreOrder);
        EZE.addListener("dontEnablePreOrder", 'click', this.dontEnablePreOrder);
        ...
        if (this.inventoryChecked) {
            try {
                ...
                if (EZD.hasClass(document.getElementById("shoppingURL"), 'fadedOut')) {
                    BD_MASTER.fadeOut(document.getElementById("shoppingURL"));
                }
            } catch (e) {}
        }
        var currDDs = EZD.getElementsByClassName("currencySelect", 'select', "stepOne")[0];
        this.itemInfo.currency = currDDs.options[currDDs.selectedIndex].value;
        this.itemInfo.currency = (EZD.hasClass(stepTwo, 'reportingDisabled')) ? 'disabled' : this.itemInfo.currency;
        BD_CBUTTON.buildDropdownInfo(0);
    },
    ...
    pickDropDown: function(e) {
        ...
        var options = EZD.getElementsByClassName("lbOption", "input", lightboxChoiceBody);
        var optionsLen = options.length;
        for (var i = 0; i < optionsLen; i++) {
    ...
    saveInvData: function() {
        ...
        var rows = EZD.getElementsByClassName("inventory-table-row", "div", parentTable);
        if (rows) {
        ...
        if (!BD_STEP2.initialLoad && document.getElementById('trackByOption')) {
            ...
            if (ddi.length > 0) {
                ...
                if (document.getElementById('trackByItem').checked && (BD_STEP2.inventoryChecked || BD_STEP2.PNLChecked)) {
                    if (ddi.length > 1) {
                        try {
                            EZD.removeClass(ddlink, 'accessAid');
                        } catch (e) {}
                    } else {
                        try {
                            EZD.addClass(ddlink, 'accessAid');
                    ...
                    if (BD_STEP2.inventoryChecked || BD_STEP2.PNLChecked) {
                        ...
                        EZD.removeClass(byOptionTable, 'accessAid');
                ...
                try {
                    ...
                    EZD.addClass(byOptionTable, 'accessAid');
                } catch (e) {}
                BD_STEP2.withPrice = false;
            }
        } else {
            var rows = EZD.getElementsByClassName("inventory-table-row", "div", byOptionTableBody);
            ...
            if (BD_STEP2.dropDowns.length > 1 && EZD.hasClass(ddlink, 'accessAid') && !withPrice) {
                EZD.removeClass(ddlink, 'accessAid');
    ...
    chooseAnotherDropDown: function(e) {
        ...
        if (tempDD != null) {
            if (BD_STEP2.ddLightbox == null) {
                BD_STEP2.ddLightbox = new EZP.util.Lightbox("ddLightbox");
            }
            var lightboxChoiceBody = document.getElementById("lightboxChoiceBody");
            ...
            for (var item in tempDD) {
                ...
                var escTitle = EZP.util.escapeHTML(title);
                var escsubItem = EZP.util.escapeHTML(subItem);
    ...
    trackByItem: function() {
        ...
        var rowsLen = rows.length;
        if (rowsLen < 1) {
            rows = EZD.getElementsByClassName("inventory-table-row", "div", optionsTable);
        }
        if (rowsLen > 0) {
        ...
        try {
            if (BD_STEP2.inventoryChecked || BD_STEP2.PNLChecked) {
                ...
                BD_MASTER.fadeIn(parentTable);
                var PNLItems = EZD.getElementsByClassName("PNLRelated", '', parentTable);
                var invItems = EZD.getElementsByClassName("invRelated", '', parentTable);
    ...
    trackByOption: function() {
        ...
        rows = EZD.getElementsByClassName("inventory-table-row", "div", parentTable);
        rowsLen = rows.length;
        if (rowsLen < 2) {
            var rows = EZD.getElementsByClassName("inventory-table-row", "div", itemTable);
        ...
        if (!BD_STEP2.initialLoad) {
            ...
            var PNLItems = EZD.getElementsByClassName("PNLRelated", '', parentTable);
            var invItems = EZD.getElementsByClassName("invRelated", '', parentTable);
            var parentDisabled = EZD.hasClass(byOptionTable, 'fadedOut');
            if (BD_STEP2.inventoryChecked) {
                BD_MASTER.fadeIn(invItems);
            } else {
                if (!parentDisabled) {
    ...
    itemDetailsChangeSubscriber: function(e, obj) {
        ...
        if (obj[0].button_type && invOpt != null) {
            ...
            case "donations":
                ...
                break;
            default:
                if (!EZD.hasClass(soldOutOpt, 'fadedOut')) {
        ...
        if ((obj[0].item_price_currency || obj[0].ddp_option_currency) && BD_STEP2.itemInfo.currency != 'disabled') {
            ...
            var fieldsToUpdate = EZD.getElementsByClassName('right-edge', 'div', 'inventoryTable');
        ...
        if (obj[0].product_name) {
            ...
            var itemName = EZP.util.escapeHTML(itemNameEsc);
        ...
        if (obj[0].product_id || obj[0].subscription_id) {
            var itemId = EZD.getElementsByClassName('type-text', 'input', 'byItemTableBody')[0];
            if (obj[0].subscription_id) {
                try {
    ...
    toggleOptions: function() {
        ...
        var kiddies = EZD.getElementsBy(function(ele) {
    ...
    toggleTables: function() {
        ...
        var PNLItems = EZD.getElementsByClassName("PNLRelated", '', daddy);
        var invItems = EZD.getElementsByClassName("invRelated", '', daddy);
        ...
        if (!BD_STEP2.inventoryChecked && !BD_STEP2.PNLChecked) {
            ...
            if (BD_STEP2.byItem) {
                BD_MASTER.fadeIn(byItemTable);
                BD_MASTER.fadeIn(byItemTableBody);
                if (!EZD.hasClass(document.getElementById('chooseAnotherDropDown'), 'accessAid') || BD_STEP2.dropDowns.length > 0) {
                    BD_MASTER.fadeIn(byOptionTable);
                    EZD.removeClass(byOptionTable, 'accessAid');
                ...
                BD_MASTER.fadeOut(byOptionTableBody);
                PNLItems = EZD.getElementsByClassName("PNLRelated", '', byItemTable);
                invItems = EZD.getElementsByClassName("invRelated", '', byItemTable);
            } else {
                BD_MASTER.fadeIn(byOptionTable);
                EZD.removeClass(byOptionTable, 'accessAid');
                BD_MASTER.fadeIn(byOptionTableBody);
                BD_MASTER.fadeIn(byItemTable);
                PNLItems = EZD.getElementsByClassName("PNLRelated", '', byOptionTable);
                invItems = EZD.getElementsByClassName("invRelated", '', byOptionTable);
            }
            if (BD_STEP2.inventoryChecked) {
                BD_MASTER.fadeIn(invItems);
                BD_MASTER.fadeIn(soldOutOption);
...
EZP.Merchant.EZEBTN.StepThree = {
    init: function() {
        ...
        EZE.addListener(cancellationCheckbox, 'change', this.toggleTextfield, cancellationRedirectURL, false);
        ...
        EZE.addListener(successfulCheckbox, 'change', this.toggleTextfield, successfulRedirectURL, false);
        var variablesTextarea = document.getElementById('variablesTextarea');
        var addVariables = document.getElementById("addVariables");
        EZE.addListener(variablesTextarea, 'keyup', this.checkCheckbox, addVariables, false);
...
BD_STEP2 = EZP.Merchant.EZEBTN.StepTwo;
BD_STEP3 = EZP.Merchant.EZEBTN.StepThree;
BD_STEP2.init();
BD_STEP3.init();
