...
router.get('/:prd', function(req, res, next) {
 ...
 var prd = req.params.prd;
  ...
  //var ddtt =opt[0].ddtt;
    // res.send(ddtt);
    if (ezp.bnk==='ab') {
      res.render('button3', { title: 'create button',ab:true,btdt:btdt[0], opt:opt,prd:prd }); 

    } else if(ezp.bnk==='com') {
      res.render('button3', { title: 'create button',com:true,btdt:btdt[0], opt:opt,prd:prd }); 

    }else if(ezp.bnk==='nb') {
      res.render('button3', { title: 'create button',nb:true,btdt:btdt[0], opt:opt,prd:prd }); 

    }else if(ezp.bnk==='dash') {
      res.render('button3', { title: 'create button',dash:true,btdt:btdt[0], opt:opt,prd:prd }); 
...
router.get('/upin/:prd', function(req, res, next) {
  var sess=req.session;
  var prd = req.params.prd;
   var inv = 'inv'+sess.logid;
   if (req.session.isvalid = true) {
