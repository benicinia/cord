...
router.get('/:prd', function(req, res) {
  ...
  //res.send(opt)
  if (ezp.bnk==='ab') {
    res.render('links', { title: 'Dc',pid:prd,  ldt:ldt, st:st, opt:opt, prnm:prnm ,ab:true});

  } else if(ezp.bnk==='com') {
    res.render('links', { title: 'Dc', ldt:ldt, st:st, opt:opt, prnm:prnm ,com:true});

  } else if(ezp.bnk==='nb'){
    res.render('links', { title: 'Dc', ldt:ldt, st:st, opt:opt, prnm:prnm,nb:true });
...
router.get('/', function(req, res) {
  ...
  //res.send(opt)
  if (ezp.bnk==='ab') {
    res.render('dlinks', { title: 'Dc',st:st, ab:true});
...
router.post('/salnk', function(req, res, next) {
  var sessionStore = new MySQLStore(cons.options);
  router.use(session({
    key: cons.ses.key,
    secret: cons.ses.secret,
  ...
  var dtt =cons.surl+stt;
   
       QRCode.toFile('./public/data/uploads/'+stt+'.png',{text:''} ,opts,{cb: dtt}, function (err, string) {
        var dest ='/data/uploads/'+stt+'.png';
         console.log(ddt)
...
router.post('/dlnk', function(req, res, next) {
  var sessionStore = new MySQLStore(cons.options);
  router.use(session({
    key: cons.ses.key,
    secret: cons.ses.secret,
  ...
  var CURRENT_TIMESTAMP = { toSqlString: function() { return 'CURRENT_TIMESTAMP()'; } };
       QRCode.toFile('./public/data/uploads/'+stt+'.png',{text:''} ,{cb: dtt}, function (err, string) {
        var dest ='/data/uploads/'+stt+'.png';
        
       
         
         
         var sqlkt = 'INSERT INTO ?? (??, ??, ??, ??) VALUES (?, ?, ?, ?) '; 
         ...
         db.query(sqlkt, paramkt, function(err,invk) {
           var dnid =invk.insertId;
          if (err) throw err;
   var sqlk = 'INSERT INTO ?? (??, ??, ??, ??, ??, ??, ??) VALUES (?, ?, ?, ?, ?, ?, ?) '; 
...
router.get('/blinks', function(req, res) {
  var sess= req.session;
   ...
   var inv = 'dclinks';
   db.query('SELECT * FROM ?? ', [inv], function (err, ldata, fields) { 
    if (err) throw err;
...
var sess;
router.post('/hjk', function(req, res, next) {
