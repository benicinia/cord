...
router.get('/', function(req, res, next) {
  var sess =req.session;
    ...
    var msgt= 'messages'+req.session.logid;
    db.query('SELECT * FROM customer where email= ? ', [req.session.email], function (err, data, fields) {
      db.query('SELECT * FROM ?? ORDER BY `trans_id` DESC ', [pa6], function (err, dat, fields) {
        db.query('SELECT * FROM ?? ORDER BY `tba` DESC ', [por], function (err, da, fields) {
          db.query('SELECT * FROM ?? ORDER BY `prd` DESC ', [invt], function (err, din, fields) {
           db.query('SELECT * FROM ?? ORDER BY `message_id` DESC ', [msgt], function (err, msgn, fields) {    
     ...
     db.query(sqlco, [por,'snn','0'], function (err, kino, fields) {
      ...
      db.query(sqlcmg, [msgt,'snn','0'], function (err, msgy, fields) {
if(!err)
      try {
...
//var pp='';
  ...
  var home = 'cards';
var title ='Dashboard';
var usrnv= [{bbtn:ezp.btns[sess.ln],addp:ezp.addp[sess.ln],withd:ezp.withd[sess.ln],lnk:ezp.lnk[sess.ln],invv:ezp.inv[sess.ln],dash:ezp.dashb[sess.ln],sndm:ezp.sndm[sess.ln],inv:ezp.inv[sess.ln],ana:ezp.ana[sess.ln]}]
sess.usrnv=usrnv
...
{var pp =pp} var tyEr;
 //res.send(lan)
 tyEr ={dlnk:ezp.dlnk[sess.ln],plnk:ezp.plnk[sess.ln],blg:ezp.blg[sess.ln],[cmgt]:cmg,msgn:msgn,[ppf]:pmg,p:'0',bbtn:ezp.btns[sess.ln],addp:ezp.addp[sess.ln],withd:ezp.withd[sess.ln],lnk:ezp.lnk[sess.ln],
  ...
  task:ezp.task[sess.ln],tran:ezp.transactions[sess.ln],[sess.ln]:true,[bnz]:true,kin:coun,kino:counto, 
  invt:din[0],[ppt]:true, [gent]:true, 
...
res.render(home, tyEr);
  ...
  console.log(err)
  tyE ={err:err,[sess.ln]:true,[bnz]:true,kin:coun,kino:counto, invt:din[0],[ppt]:true, [gent]:true, 
    title: title,[ezp.bnk]:true,bal:bal,userData: data[0],bal:bal, passData: dat[0], orderData: da[0]}
   //res.send(tyre)
  res.render(home, tyE);
    ...
    router.get('/am', function(req, res, next) {
     ...
     /*  var FORMATS = {
      ...
      var date = currentDate.getUTCDay();
       ...
       var fgf= '2021-08-14 20:59:18';
       var h =toDate(fgf);
       res.render('cards-am', { title: 'Dashboard',bal:bal, userData: data[0], passData: dat[0], orderData: da[0]});
      }); 
