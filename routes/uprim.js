...
//var tf = require('@tensorflow/yfjs-node');
...
var storage = multer.diskStorage({
  destination: (req, file, callBack) => {
      callBack(null, './public/data/uploads/')     // './public/images/' directory name where save the file
  },
...
router.post('/photos', upload.single('image'), function (req, res, next) {
  ...
  if (!req.file) {
    ...
    var src = req.files['image']['0'];
    
     // photos
     var imgsrc = '/data/uploads/' + req.file.filename;
    var insertData = 'INSERT INTO ?? (??) VALUES (?) ';  
  ...
  var parp = [
    ...
    sdb.query(insertData, parp, (err, result) => {
        if (err) throw err
...
router.post('/', upload.single('image'), function(req, res, next) {
  ...
  if (!req.file) {
    ...
    var imgsrc = '/data/uploads/' + req.file.filename;
   /* promise.all([cocosd.load(),fs.readFile(imgsrc)])
    .then((results) =>{
      var model =results[0];
      ...
      .then((pridictions => {
        console.log(Json.stringify(pridictions,null,2))
      })
...
router.post('/prim', upload.single('image'), function(req, res, next) {
  var map=req.body
  var opindx =map.optindx;
  ...
  if (!req.file) {
    ...
    console.log(req.file.filename)
    var imgsrc = '/data/uploads/' + req.file.filename;
    var sqlinv = 'SELECT ?? AS optnz FROM ?? WHERE ?? = ?'; //GET inventory
                          ...
                          db.query(sqlinv, parainv, function (err, resuinv, fields) {
                            ...
                            objIndex = qual.findIndex((obj => obj.index == opindx));
                            var uqua= qual[objIndex].imgur;
                             //uqua =imgsrc;
                               qual[objIndex].imgur = imgsrc;
                            
                           // console.log(uqua)
                            var quakl= JSON.stringify(qual)
