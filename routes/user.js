...
router.get('/stats/:id', function(req, res) {
  ...
  var stv = 'lstat'+sess.logid;
   db.query('SELECT * FROM ?? WHERE url_id = ?', [stv,idd], function (err, stv, fields) {
...
//sess.isvalid = false;
 ...
 var  inv='inv'+sess.logid;
  db.query('SELECT * FROM ?? ORDER BY `prd` DESC',[inv],  function (err, sinv, fields) {
  ...
  if(sess.pp==='1'){
   ...
   if(sess.gen==='male'){
    ...
    var gent='g';
          ...
          shpk:ezp.shpk[sess.ln],infb:ezp.infb[sess.ln],inlrt:ezp.invlrt[sess.ln],or:ezp.orders[sess.ln],
          task:ezp.task[sess.ln],tran:ezp.transactions[sess.ln],[sess.ln]:true,[bnz]:true, 
         [ppt]:true, [gent]:true, sinv: sinv,srv:srv,
          title: title,[ezp.bnk]:true,}
          res.render(home, tyEr);
 ...
 if(sess.gen==='male'){
  ...
  var gent='g';
        ...
        {var pp =pp} var tyEr;
            ...
            tyEr ={dlnk:ezp.dlnk[sess.ln],plnk:ezp.plnk[sess.ln],blg:ezp.blg[sess.ln],[ppf]:sess.pmg,p:'5',bbtn:ezp.btns[sess.ln],addp:ezp.addp[sess.ln],withd:ezp.withd[sess.ln],lnk:ezp.lnk[sess.ln],
            invv:ezp.inv[sess.ln],dash:ezp.dashb[sess.ln],sndm:ezp.sndm[sess.ln],ana:ezp.ana[sess.ln],
            shpk:ezp.shpk[sess.ln],infb:ezp.infb[sess.ln],inlrt:ezp.invlrt[sess.ln],or:ezp.orders[sess.ln],
            task:ezp.task[sess.ln],tran:ezp.transactions[sess.ln],[sess.ln]:true,[bnz]:true, 
           [ppt]:true, [gent]:true, inv: inv,
